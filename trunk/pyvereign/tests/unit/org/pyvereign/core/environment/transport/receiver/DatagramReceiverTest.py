...
class DatagramReceiverTest(unittest.TestCase):
    ...
    def test_create_instance_with_IPv4Address(self):
        self.assertTrue(DatagramReceiver(IPv4Address("192.168.1.2", 5050), NetworkProtocolImpl({})))
        
    def test_create_instance_with_BindIPv4Address(self):
        self.assertTrue(DatagramReceiver(BindIPv4Address(5050), NetworkProtocolImpl({})))
        
    def test_try_create_instance_with_BroadcastIPv4Address(self):
        self.assertRaises(TransportError, DatagramReceiver, BroadcastIPv4Address(5050), NetworkProtocolImpl({}))
        
    def test_open_receiver(self):
        receiver = DatagramReceiver(BindIPv4Address(5050), NetworkProtocolImpl({}))
        self.assertTrue(receiver.open())
        self.assertTrue(receiver.close())
        
    def test_bind_receiver(self):
        receiver = DatagramReceiver(BindIPv4Address(5053), NetworkProtocolImpl({}))
        self.assertTrue(receiver.open())
        self.assertTrue(receiver.bind())
        self.assertTrue(receiver.close())
        
    def test_receive_stream(self):
        receiver = DatagramReceiver(BindIPv4Address(5051), NetworkProtocolImpl({}))
        self.assertTrue(receiver.open())
    ...
    def test_try_bind_receiver_with_invalid_address(self):
        receiver = DatagramReceiver(IPv4Address("192.168", 5051), NetworkProtocolImpl({}))
        self.assertTrue(receiver.open())
